...
calculate_median <- function(df) {
  smedian <- \(d, i, ...) d[i,] %>% pull(duration) %>% median()
  bdf <- boot(data = df, statistic = smedian, R = 4000)
...
dh_median_ic <- grouped_dh_latency %>% map_df(~calculate_median(.))
...
df_latency %>%
  plot_ecdf_latency(b = "dynamic-html", c = "gc", heaps = c("2g2c", "20g2c"), gcs = all_gcs, nticks = 5)

df_latency %>%
  plot_ecdf_latency(b = "dynamic-html", c = "gc", heaps = c("4g2c", "20g2c"), gcs = all_gcs, nticks = 5)
```
...
```{r, fig.width=8,fig.height=6}
df_latency %>%
  plot_ecdf_latency(b = "thumbnailer", c = "gc", heaps = c("2g2c", "20g2c"), gcs = all_gcs, nticks = 5)
```
