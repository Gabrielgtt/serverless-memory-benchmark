...
alloc_info <- df_latency_epsilon %>%
  ...
  ungroup() %>%
  distinct(benchmark, mean_alloc_mb, mean_duration_s, mean_alloc_per_sec)
...
t1 <- alloc_info %>%
  ggtexttable(rows = NULL, theme = ttheme("minimal", tbody.style = tbody_style(hjust=1, x=0.9)), 
   ...
   table_cell_bg(row = 5:5, column = 1:ncol(alloc_info), fill = "lightgreen") %>%
   table_cell_bg(row = 6:6, column = 1:ncol(alloc_info), fill = "orange2") %>%
   table_cell_bg(row = 7:7, column = 1:ncol(alloc_info), fill = "gold1")

alloc_groups <- tibble(Groups = c("Minimal", "Medium", "Heavy-fast", "Heavy-Slow"))
...
t2 <- alloc_groups %>%
  ggtexttable(rows = NULL) %>%
   ...
   table_cell_bg(row = 5, column = 1, fill = "gold1", linewidth = 0)
